      ...
      <div class="col-12 our-services-section">
        <div class="row">
            <div class="col-11 mx-auto">
              @php
              $tit=explode(',',$aboutus->heading);
              $des=explode(',',$aboutus->descp);
              $memname=explode(',',$aboutus->MemberName);
              $memdes=explode(',',$aboutus->MemberDes);
              $img = [
              ...
              @foreach($tit as $ti)
        <h2 class="pt-5 pb-4 text-start">{{$ti}}</h2>
              @endforeach
      ...
      <div class="col-12 our-team-box">
        <div class="row">
          <div id="ourTeam" class="col-11 mx-auto">
            <h1 class="pt-5 pb-4 text-center">{{$aboutus->TeamTitle}}</h1>
        <ul class="our-team-list">

        @for($i=0;$i<count($img);$i++)
          <li class="m-0 m-lg-4 text-center">
            <img src="{{$img[$i]}}" alt="">
            <p class="team-title pt-3 pb-1 mb-0"><strong>{{$memname[$i]}}</strong></p>
            <p>{{$memdes[$i]}}</p>
          </li>
            @endfor
        </ul>
